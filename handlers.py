...
async def add(message, command):
    ...
    if not is_valid_ipv4_address(ip_address):
        ...
        response = subprocess.run(["ufw", "allow", "from", f"{ip_address}", "proto", "tcp", "to", "any", "port", "80"])
        response1 = subprocess.run(["ufw", "allow", "from", f"{ip_address}", "proto", "tcp", "to", "any", "port", "443"])
